...
- Can auto-fill buyer-paid shipping from the average shipping amount on used eBay comparables when eBay exposes shipping.
- Browser action popup can show the panel manually and toggle automatic panel opening on Marketplace listing pages.
...
Shipping is split into two fields: `Buyer Shipping` is the amount the buyer pays and is included in the fee base, while `Shipping Cost` is your postage/label expense and reduces profit. `Shipping unknown` means the extension could not parse shipping or delivery from the sold-search row.
...
7. Click the extension icon and turn off `Auto-open on Marketplace listings`.
8. Refresh the listing and confirm the panel does not appear automatically.
9. Click the extension icon, then click `Show Panel`, and confirm the panel appears.
10. Turn auto-open back on if you want the panel to appear by default.
11. Confirm the panel shows title, price, image URL, and listing URL.
12. Open DevTools, then check the Console for logs prefixed with `[Marketplace Flip Scanner]`.
13. Click `Check eBay`.
14. Confirm Chrome opens an eBay background tab with sold and completed listings for the captured title.
15. Wait for the eBay page to finish loading.
16. Return to the original Facebook Marketplace tab.
17. Confirm the `eBay Sold Comparables` section appears in the floating panel.
18. Review the raw comp rows. Each row is marked `Used` or `Excluded`, with match percentage and exclusion reason when applicable.
19. Review the `Profitability` section.
20. Change fee, promoted listing, shipping, packaging, or other expense inputs and confirm the net/profit/ROI update.
21. Reload the Marketplace page and confirm those assumption inputs are remembered.
22. Confirm the captured Marketplace payload, eBay URL, and extracted comparables are logged in the Console.
